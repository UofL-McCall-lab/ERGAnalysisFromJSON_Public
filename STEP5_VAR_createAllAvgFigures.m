% Script to export all average figures from a STEP4 mat (McCall lab version)
...
%% Initialize typeNames (McCall data) and x/y axes labels
...
for rowN = 1:numel(formattedMat)
    ...
    %% aWave/TTT text
    text(cAxes, 0.15, 0.21, {aWaveTxt, TTTTxt}, 'Units', 'normalized', 'FontSize', 16, 'Color', 'r'); 
    %% bWave/TTP text
    text(cAxes, 0.4, 0.95, {bWaveTxt, TTPTxt}, 'Units', 'normalized', 'FontSize', 16, 'Color', 'r');
    hold(cAxes, 'off');
    xlabel(xAxLabel);
    ylabel(yAxLabel);  
