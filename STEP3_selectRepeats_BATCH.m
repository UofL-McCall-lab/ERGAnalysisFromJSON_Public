%{
% STEP3_selectRepeats_BATCH.m
% PURPOSE: Find and select repeats in ERG waveform data. Reworked version
%   of older live script (new as of 08-2024). Batch version to process
%   multiple STEP2 mats in a folder in a row.
%
...
allMats = getFiles_F(path, '.mat');
if isempty(allMats{1, 1}); beep; disp('No .mats found in input folder. Closing...'); return; end
allMats = allMats(contains(allMats(:, 2), '_step2Mat'), :); % Now clear any that don't have '_step2Mat'
fprintf('Found %i STEP2 .mats for repeat removal\n', size(allMats, 1));
%% Loop over all found STEP2 mats
for fileN = 1:size(allMats, 1)
    fprintf('Processing %s\n', allMats{fileN, 2});
    clearvars formattedMat
    load(allMats{fileN, 1}); % Loads struct called formattedMat
    ...
    for N = 1:size(startGroups, 1)
        ...
        if oddEqCheck && evenEqCheck % All groups identical. Pick first pair, notify user, then skip to next set
            ...
            for remN = 1:numel(toRemRowsID)
                formattedMat(toRemRowsID(remN)).toUse = false;
                formattedMat(toRemRowsID(remN)).remReason = 'IDENTICALGROUPS';
            end
            continue
        end
        ...
        for remN = 1:numel(toRemRows)
            formattedMat(toRemRows(remN)).toUse = false;
            formattedMat(toRemRows(remN)).remReason = 'USERREMOVED';
        end
        fprintf('\n');
    end
