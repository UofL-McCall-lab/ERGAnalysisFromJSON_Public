%{
% VAR_STEP3_selectRepeatsCustomPairs.m
% PURPOSE: Find and select repeats in ERG waveform data. Modified to allow
%   custom pairs (see issue #43 on github for specifics)
%
...
for N = 1:size(startGroups, 1)
    ...
    for plotN = 1:numPlots % Loop per row of tiledLayout figure
        endIdx = plotN*2; % Idx in toUseRows
        startIdx = endIdx-1;
        endN = toUseRows(endIdx);
        startN = toUseRows(startIdx);
    ...
    promptDlg = strcat('REPEAT', num2str(numUserSelRepeats), '-Hold CTRL, select pair');
    [selRep, validSelection] = listdlg('ListString', string(toUseRows), 'PromptString', promptDlg, 'SelectionMode', 'multiple');
    %^ Does prevent user interacting with plot (zoom etc). Might not be an issue with maximized WindowState
    if numel(selRep) ~= 2
        beep
        fprintf("ERROR:: Did not select exactly two plots (a pair). Closing program...\n");
        return
    end
    close all
    ...
    % selRep is the plot number, not the row to use
    selectedRows = toUseRows(sort(selRep)); % Sort to always make ascending since listdlg('multiple') retains selection ordering
    toRemRows = toUseRows(~ismember(toUseRows, selectedRows));
    for remN = 1:numel(toRemRows)
        formattedMat(toRemRows(remN)).toUse = false; %#ok<SAGROW> 
        formattedMat(toRemRows(remN)).remReason = 'USERREMOVED_CUSTOMPAIR'; %#ok<SAGROW> 
    end
    fprintf('\n');
end
...
outFile = replace(file, ['_' splitFile{end}], '_customPair_step3Mat.mat');
